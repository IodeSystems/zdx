...
import { useCounted, useCountedGrouped, useCountedTagKeys, useCountedTagValues } from '../../../api'

function TagFilterSelect({ slug, tagKey, value, onChange }: {
  slug: string; tagKey: string; value: string; onChange: (v: string) => void
}) {
  const { data } = useCountedTagValues(slug, tagKey)
...
function CountersPage() {
  ...
  const { data: keysData } = useCountedTagKeys(slug)
  ...
  const { data: countedData, isLoading } = useCounted(slug, undefined, undefined, activeFilters)
  const { data: groupedData, isLoading: groupLoading } = useCountedGrouped(
    slug, groupBy, activeFilters
  )

  const data = countedData?.items
  const grouped = groupedData?.items

  return (
    <>
      <Typography variant="h6" sx={{ fontWeight: 600, mb: 2 }}>Counters</Typography>
                ...
                <TableBody>
                  {(grouped ?? []).map(item => (
                    <TableRow key={item.group_value} hover>
                      ...
                      <TableCell align="right">{item.max_value}</TableCell>
                      <TableCell align="right">{item.avg_value}</TableCell>
                      <TableCell align="right">{item.sum_count}</TableCell>
                      <TableCell align="right">{item.sum_total_value}</TableCell>
                    </TableRow>
                  ))}
                </TableBody>
              </Table>
      ...
      {isLoading ? <CircularProgress sx={{ m: 4 }} /> :
       (!data || data.length === 0) ? <Typography color="text.secondary">No counter entries.</Typography> : (
...
export const Route = createFileRoute('/project/$slug/counters')({
