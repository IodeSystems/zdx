...
func collectTechMetrics(projectRoot string) TechMetrics {
	m := TechMetrics{}
...
func collectGitChurn(projectRoot, sinceDate string) (commits int, filesChanged int) {
	...
	out, err = exec.Command("git", "-C", projectRoot, "diff", "--name-only", "--since="+sinceDate, "HEAD").Output()
	if err == nil {
...
func metricsToJSON(m TechMetrics) string {
	b, _ := json.Marshal(m)
	return string(b)
}
...
func parseTechMetrics(stateJSON string) (TechMetrics, bool) {
...
func formatMetricsSummary(m TechMetrics, deltas []MetricDelta) string {
