...
// runMCPContainerSingle provisions ONE slot, runs ONE managed session
// against it (claim+work+release via RunManagedSession), tears the slot
// down. Composes the same provisionSlot primitive used by the loop path,
// closing the gap that previously errored at agent.go's bare `dx agent
// --container=docker` ("not yet wired").
//
// Differences from runMCPContainerLoop: no orchestrator tracelog scope,
// no signal-driven os.Exit (defer cleanup is enough for one slot), no
// fan-out — exactly one slot, exactly one session.
func runMCPContainerSingle(parentCtx context.Context, providerName string, opts ProviderOpts) error {
	if opts.RC.slug == "" {
		return fmt.Errorf("--container=docker requires a project config with a remote slug")
	}
	...
	// Default a stable alias if the operator didn't pass --alias. Loop mode
	// gets one auto-generated by RunManagedLoop, but single-session paths
	// don't pass through that helper, so we mirror its naming here.
	if opts.Alias == "" {
	...
	slot, err := provisionSlot(ctx, providerName, imageTag, cwd, opts, 0, agentCfg, envPairs)
	if err != nil {
		return err
	}
	defer slot.Cleanup()
	...
	// defer slot.Cleanup() then tears the slot down cleanly. No os.Exit
	// dance (loop mode needs that for parallel goroutine drain; single
	// session is sequential).
	...
	sessionOpts := opts
	sessionOpts.MCPCommand = slot.MCPCmd
	fmt.Printf("[%s] container-single mode: slot=%s mount=%s\n",
		time.Now().Format(time.RFC3339), slot.Name, slot.Mount)
	return RunManagedSession(ctx, providerName, sessionOpts)
}
...
func runMCPContainerLoop(parentCtx context.Context, providerName string, opts ProviderOpts) error {
	...
	// Provision N slots through the shared `provisionSlot` primitive — the
	// same one the single-session path uses. Each slot gets its own
	// ~/.zdx/projects/<slug>/slots/<alias>-<i>/ worktree on a fresh
	// wip/<alias>-<i> branch and its own docker container running
	// `sleep infinity`. Without per-slot worktrees, every slot (and the
	// operator) would share the same working tree and stomp each other's
	// edits silently.
	slots := make([]*provisionedSlot, 0, maxSlots)
	cleanupAll := func() {
		for _, s := range slots {
			s.Cleanup()
		}
	...
	for i := 0; i < maxSlots; i++ {
		emit("slot.starting", "slot_index", i)
		s, err := provisionSlot(ctx, providerName, imageTag, cwd, opts, i, agentCfg, envPairs)
		if err != nil {
			emit("slot.provision_failed", "slot_index", i, "err", err.Error())
			cleanupAll()
			return err
		}
		...
		slotMu.Unlock()
		logf("slot %d started: %s", i, s.Name)
		emit("slot.started", "slot_index", i, "name", s.Name, "mount", s.Mount, "branch", s.Branch)
	}
	defer cleanupAll()
	cleanupOnSignal = cleanupAll

	var wg sync.WaitGroup
	for i, s := range slots {
		wg.Add(1)
		go func(slot int, s *provisionedSlot) {
			defer wg.Done()
			slotOpts := opts
			slotOpts.Alias = fmt.Sprintf("%s-%d", opts.Alias, slot)
			slotOpts.ClusterID = clusterID
			slotOpts.MCPCommand = s.MCPCmd
