...
// collectBranchState collects current git state for the release call.
// Returns nil if claimBaseSHA is empty or git is unavailable.
func collectBranchState(claimBaseSHA string) map[string]any {
	if claimBaseSHA == "" {
		return nil
	}
	...
	bs := map[string]any{
		"head_sha":    strings.TrimSpace(string(headOut)),
		"head_branch": strings.TrimSpace(string(branchOut)),
		"tree_clean":  len(strings.TrimSpace(string(porcelain))) == 0,
	}
	logOut, _ := exec.Command("git", "log", claimBaseSHA+"..HEAD", "--format=%H").Output()
	if raw := strings.TrimSpace(string(logOut)); raw != "" {
		bs["commits_since_base"] = strings.Split(raw, "\n")
	}
	return bs
}
...
func releaseTodo(rc remoteConfig, todoID int32, agentID, sessionID, claimBaseSHA string, resolve bool) releaseResult {
	body := map[string]any{
		"id":         todoID,
		"agent_id":   agentID,
		"resolve":    resolve,
	...
	if bs := collectBranchState(claimBaseSHA); bs != nil {
