		...
		func(ctx context.Context, in *struct {
			...
			if branch == "dev" {
				...
				for _, targetBranch := range unresolved {
					if _, ok, bErr := h.createBackportTask(ctx, p.ID, p.Slug, "dev", issueID, issueRow.Title, targetBranch, "auto-generated by IS-825 trigger 1 on dev resolution"); ok && bErr == nil {
						created++
					}
				}
				if created > 0 {
